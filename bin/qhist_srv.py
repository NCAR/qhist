...
def get_log_data(logs, events, include, exclude, qhs_core):
    out_data    = []
    grep_cmd    = ["grep", "-a", "-h"]
...
def get_time_bounds(args, qhs_core, qhs_sys):
    err = ""
    if args["period"]:
        try:
            if '-' in args["period"]:
...
def format_job_times(job, fmt_type, qhs_core):
...
def format_job_numerics(job, fmt_type, delta_unit, qhs_core):
...
def process_job(job_data, qhs_core):
...
def process_log(log_data, events, wait_limit, qhs_core):
    err = ""
    jobs        = []
    ...
    proc_job = lambda x: process_job(x, qhs_core)
    jobs = list(map(proc_job, jobs))

    return (jobs, err)
    
def print_list(jobs, my_fields, job_stats, qhs_core):
    msg = ""
...
def compute_stats(jobs, qhs_core):
...
#
    ...
    if args["format"]:
        ...
        try:
            args["format"].format(**qhs_core["long_labels"])
    ...
    (bounds, tmp_err)      = get_time_bounds(args, qhs_core, qhs_sys)
    resp['err'] += tmp_err
    loop_date   = bounds[0]
    pbs_logs    = []
    jobs        = []
    ...
    status, log_data = get_log_data(pbs_logs, args["events"], include, exclude, qhs_core)
    
    if status > 1:
        resp['err'] += "Warning: some PBS log files could not be accessed for specified time period" + "\n"

    if len(log_data) > 0:
        (jobs, tmp_err) = process_log(log_data, args["events"], args["wait"], qhs_core)
        resp['err'] += tmp_err
    ...
    if len(jobs) > 0:
        if args["brief"]:
            ...
            if args["average"]:
                job_stats = compute_stats(jobs, qhs_core)
                format_job_numerics(job_stats, "avg_fmt", args["time"], qhs_core)
            else:
                job_stats = None
            ...
            if args["list"] or args["csv"]:
                ...
                for job in jobs:
                    format_job_times(job, fmt_type, qhs_core)
                    format_job_numerics(job, "data_fmt", args["time"], qhs_core)
                ...
                if args["list"]:
                    resp['msg'] += print_list(jobs, my_fields, job_stats, qhs_core)
                else:
                    args["nodes"]  = False
